...
const PASSTHROUGH_KEYS = ["PATH", "Path", "SystemRoot", "WINDIR", "TEMP", "TMP", "PATHEXT", "ComSpec"];
...
export async function runNodeScript(source, { env = {}, cwd = process.cwd(), timeoutMs = 30_000 } = {}) {
  ...
  const result = await new Promise((resolve, reject) => {
    ...
    child.once("close", (code, signal) => { clearTimeout(timer); finish({ code, signal, timedOut }); });
  });
...
export async function createOwnedDatabase() {
  ...
  if (!adminUrl) throw new Error("WQT_TEST_PG_ADMIN_URL is required for PostgreSQL tests");
...
export async function dropOwnedDatabase({ database, adminUrl }) {
  if (!database || !adminUrl || ownedDatabases.get(database) !== adminUrl) {
    throw new Error("Refusing to drop a database not created by this test helper");
  }
  const { Client } = await import("pg");
