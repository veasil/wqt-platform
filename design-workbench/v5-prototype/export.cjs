...
if (path.resolve(source) !== __dirname) {
  for(const name of ['index.html','styles.css','app.js','interface.js','interface.css']) fs.copyFileSync(path.join(source,name),path.join(__dirname,name));
...
html = html.replace('<script src="interface.js"></script>',()=>'<script>'+embed(fs.readFileSync(path.join(__dirname,'interface.js'),'utf8'))+'</script>');
html = html.replace('<link rel="stylesheet" href="interface.css">',()=>'<style>'+fs.readFileSync(path.join(__dirname,'interface.css'),'utf8')+'</style>');
