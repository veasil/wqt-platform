...
默认使用 `npm run change:new -- WQT-004 short-topic` 创建 `docs/changes/WQT-004-short-topic/` 的 spec、plan、evidence。小变更使用[单文件模板](docs/development/change-template.md)，无需为每个步骤创建独立文档。编号必须唯一；详见[制品入口](docs/changes/README.md)。vNext 已规定的制品路径和需求 ID 继续沿用，不另外建立冲突标准。模板中的“待确认”不是批准。
