...
import { joinActivity } from "../sessions/service.js";
import { sessionEndpoint } from "../sessions/routes.js";
...
export function registerActivityRoutes(app) {
  ...
  app.post(
    ...
    requireRole("boss"),
    async (req, res) => {
      const { name, organizer, started_at, ended_at, category } =
        req.body || {};
  ...
  app.post(
    ...
    sessionEndpoint(async (req, res) => {
      const result = await joinActivity(req.user.uid, req.body || {});
      res.json({
        ok: true,
        activity_id: result.activity.id,
        activity_name: result.activity.name,
        table_no: result.activity.table_no,
      });
    }),
  );
  ...
  app.get("/api/activities/by-code/:code", async (req, res) => {
    ...
    try {
      ...
      // Activity codes expose metadata only; session details require authenticated scope.
      res.json({ activity });
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
    ...
    async (req, res) => {
      ...
      try {
        const user = await dbGet(
          "SELECT watcher_level FROM users WHERE id = ?",
          [id],
        );
