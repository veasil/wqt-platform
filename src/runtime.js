...
async function initializeRuntime() {
  ...
  {
    const groupCount = await cardsDbGet(
      "SELECT COUNT(*) as c FROM card_groups",
    );
    if (groupCount.c === 0) {
      ...
      const raw = fs.readFileSync(
        path.join(ROOT_DIR, "public", "cards-data.js"),
        "utf8",
      );
      ...
      if (match) {
        ...
        for (const c of cards) {
          ...
          const cardRow = await cardsDbGet(
            "SELECT id FROM cards WHERE key = ?",
            [c.key],
          );
  ...
  {
    const groupsNeedCalc = await cardsDbAll(
      "SELECT id, released_ids_json FROM card_groups WHERE max_scores_json IS NULL",
    );
    for (const g of groupsNeedCalc) {
      ...
      if (Object.keys(maxScores).length > 0) {
        await cardsDbRun(
          "UPDATE card_groups SET max_scores_json = ? WHERE id = ?",
          [JSON.stringify(maxScores), g.id],
        );
      }
    }
    if (groupsNeedCalc.length > 0)
      console.log(
        `✅ 已为 ${groupsNeedCalc.length} 个卡牌组补算 max_scores_json`,
      );
  }
...
function runCleanup() {
  if (!cleanupInFlight) {
    cleanupInFlight = cleanupStaleSessions().finally(() => {
...
function createOssClient(runtimeConfig) {
  const ossConfig = {
    ...
    bucket: runtimeConfig.OSS_BUCKET_NAME || "ai5000days-scoring-system-hk",
    secure: true,
  };
  if (runtimeConfig.OSS_ENDPOINT)
    ossConfig.endpoint = runtimeConfig.OSS_ENDPOINT;
  else
    ossConfig.region = (
      runtimeConfig.OSS_REGION || "oss-cn-hongkong"
    ).startsWith("oss-")
      ? runtimeConfig.OSS_REGION
      : `oss-${runtimeConfig.OSS_REGION}`;
  const client =
    runtimeConfig.ALIBABA_CLOUD_ACCESS_KEY_ID &&
    runtimeConfig.ALIBABA_CLOUD_ACCESS_KEY_SECRET
      ? new OSS(ossConfig)
      : null;
  if (client) {
...
export async function startRuntime({ port, host } = {}) {
  if (activeRuntime || startingRuntime)
...
async function startRuntimeInternal({ port, host } = {}) {
  ...
  try {
    ...
    console.log(
      `🌐 Public access via your server IP: http://127.0.0.1:${actualPort}`,
    );
    runCleanup();
